...
const liebherrModels = [
  { model: "Liebherr R906 / R914 / R924", desc: "Диагностика насосов, поворота, распределителей и цилиндров" },
  { model: "Liebherr R934 / R944 / R950 / R954", desc: "Главные насосы, ход, клапаны и гидроцилиндры" },
  { model: "Liebherr LR621 / LR631 / LR641", desc: "Гидравлические системы гусеничной техники" },
  { model: "Liebherr LTM 1060 / LTM 1100", desc: "Гидравлические контуры крановой техники" },
  { model: "Liebherr L526 / L538 / L550", desc: "Гидравлика колёсных погрузчиков" },
  { model: "Liebherr PR726 / PR736 / PR756", desc: "Гидравлика бульдозеров" },
];

const services = [
  "Выездная диагностика гидросистемы Liebherr под нагрузкой",
  "Проверка рабочего и управляющего давления",
  "Диагностика главных гидронасосов",
  "Диагностика гидромоторов поворота и хода",
  "Проверка распределителей и клапанов",
  "Диагностика гидроцилиндров и внутренних утечек",
  "Проверка электрического управления гидросистемой",
...
export default function BrandLiebherr() {
  ...
  const handleWhatsApp = () => {
    ...
    window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent("Здравствуйте! Нужна выездная диагностика Liebherr. Понимаю, что комплексная диагностика начинается от 200 000 ₸. Могу отправить модель, местонахождение и видео неисправности.")}`, "_blank");
  };

  return (
    <div className="min-h-screen bg-[#111111] text-white font-roboto">
      <SEO
        title="Диагностика и ремонт гидравлики Liebherr | ACA Hydraulic"
        description="Выездная диагностика гидравлики Liebherr: экскаваторы, погрузчики, бульдозеры и крановая техника. Давление, насосы, гидромоторы, клапаны и электрика. Диагностика от 200 000 ₸."
        keywords="ремонт гидравлики Liebherr, диагностика Liebherr R950, ремонт экскаватора Liebherr, ремонт гидронасоса Liebherr Казахстан"
        canonical="/brands/liebherr"
        breadcrumbs={[{ name: "Бренды", url: "/services" }, { name: "Liebherr", url: "/brands/liebherr" }]}
        serviceSchema={{
          serviceName: "Выездная диагностика и ремонт гидравлики Liebherr",
          serviceDescription: "Диагностика и ремонт гидравлических систем техники Liebherr на объекте по Казахстану.",
          serviceUrl: "/brands/liebherr",
          areaServed: ["Астана", "Казахстан"],
        }}
        faq={[
          {
            question: "Сколько стоит диагностика гидравлики Liebherr?",
          ...
          {
            question: "Можно ли определить неисправный насос Liebherr только по падению мощности?",
            answer: "Нет. Падение мощности может быть связано не только с насосом, но и с регулятором, клапанами, распределителем, управляющим давлением, внутренними утечками или электрическим управлением. Причину подтверждаем измерениями под нагрузкой.",
          },
          {
            question: "Выезжаете ли на объект за пределы Астаны?",
            answer: "Да. Выполняем выездную диагностику техники Liebherr по Казахстану. Условия и стоимость выезда согласуются по местонахождению техники.",
          },
        ]}
      />
      ...
      <section className="py-16 bg-[#111] border-y border-white/10"><div className="container mx-auto px-4 grid lg:grid-cols-2 gap-8"><div><h2 className="text-3xl md:text-4xl font-bold mb-4">Не называем ремонт насоса по модели без дефектовки</h2><p className="text-white/65 leading-relaxed">Даже при низком давлении причиной может быть не сам насос. Сначала проверяем давление, регулирование, управляющие контуры, распределитель, клапаны и внутренние утечки. Только после этого согласуем ремонт конкретного узла.</p></div><div className="bg-[#1a1a1a] border border-[#FFC000]/25 rounded-xl p-6"><div className="text-white/50 text-sm uppercase tracking-wider mb-2">Перед выездом подготовьте</div><ul className="space-y-3 text-white/80"><li>• модель и фото шильдика;</li><li>• местонахождение техники;</li><li>• видео симптома;</li><li>• историю предыдущих ремонтов и замен.</li></ul></div></div></section>

      <section className="py-16 bg-[#1a1a1a]"><div className="container mx-auto px-4"><h2 className="text-3xl font-bold mb-10">Модели <span className="text-[#FFC000]">Liebherr</span>, с которыми работаем</h2><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{liebherrModels.map((item,i)=><div key={i} className="bg-[#222] rounded-xl p-5 border border-white/5 hover:border-[#FFC000]/30"><h3 className="font-bold text-[#FFC000] mb-2">{item.model}</h3><p className="text-white/60 text-sm">{item.desc}</p></div>)}</div></div></section>

      <section className="py-16 bg-[#FFC000]"><div className="container mx-auto px-4 text-center max-w-3xl"><h2 className="text-3xl font-bold text-black mb-4">Проблема с гидравликой Liebherr?</h2><p className="text-black/70 mb-3 text-lg">Сначала подтверждаем фактическую причину неисправности. Ремонт и запчасти рассчитываем отдельно после диагностики.</p><p className="text-black font-bold text-xl mb-8">Комплексная диагностика — от 200 000 ₸.</p><div className="flex flex-wrap justify-center gap-4"><button onClick={handleWhatsApp} className="flex items-center gap-2 bg-black text-white px-8 py-4 rounded-lg font-bold text-lg hover:bg-gray-900"><MessageCircle className="w-6 h-6"/>WhatsApp</button><a href={`tel:${PHONE_NUMBER.replace(/\s/g, "")}`} className="flex items-center gap-2 bg-white text-black px-8 py-4 rounded-lg font-bold text-lg hover:bg-gray-100"><Phone className="w-6 h-6"/>{PHONE_NUMBER}</a></div></div></section>

      <section className="py-12 bg-[#111111]"><div className="container mx-auto px-4"><h2 className="text-xl font-bold mb-6 text-white/80">Другие бренды</h2><div className="flex flex-wrap gap-3">{[{name:"CAT",href:"/brands/cat"},{name:"Komatsu",href:"/brands/komatsu"},{name:"Hitachi",href:"/brands/hitachi"},{name:"Hyundai",href:"/brands/hyundai"},{name:"Wirtgen",href:"/brands/wirtgen"},{name:"SHANTUI",href:"/brands/shantui"},{name:"Volvo CE",href:"/brands/volvo"}].map((brand)=><Link key={brand.href} href={brand.href}><span className="inline-block bg-[#1a1a1a] border border-white/20 hover:border-[#FFC000] text-white/70 hover:text-[#FFC000] px-5 py-2 rounded-full text-sm font-medium">{brand.name}</span></Link>)}</div></div></section>

      <Dialog open={isLeadFormOpen} onOpenChange={setIsLeadFormOpen}><DialogContent className="bg-[#1a1a1a] border-white/10 text-white max-w-lg"><DialogHeader><DialogTitle className="text-white">Заявка на диагностику Liebherr</DialogTitle></DialogHeader><B2BLeadForm onSuccess={() => setIsLeadFormOpen(false)}/></DialogContent></Dialog>
    </div>
  );
}
