...
test('watch pages expose matching video metadata and useful content without JavaScript', () => {
  for (const item of cases) {
    ...
    assert.equal(schemas[0]['@type'], 'VideoObject');
    assert.equal(schemas[0].contentUrl, `https://acahydraulic.kz${item.video}`);
    assert.equal(schemas[0].mainEntityOfPage, `https://acahydraulic.kz/${route}/`);
